 ...
 * exactly what we need for the JOric GWT platform's sound sample queue.
 * 
...
public class SharedQueue {
    ...
    public native int push(Float32Array elements)/*-{
        ...
        // Queue is full.
        if ((wr + 1) % (this.@emu.joric.gwt.SharedQueue::_storage_capacity()()) === rd) {
            return 0;
        }
        
        var len = elements.length;
        var to_write = Math.min(this.@emu.joric.gwt.SharedQueue::_available_write(II)(rd, wr), len);
        var first_part = Math.min((this.@emu.joric.gwt.SharedQueue::_storage_capacity()()) - wr, to_write);
        var second_part = to_write - first_part;

        // Handles wrapping around in the buffer.
        this.@emu.joric.gwt.SharedQueue::_copy(Lcom/google/gwt/typedarrays/shared/Float32Array;ILcom/google/gwt/typedarrays/shared/Float32Array;II)(
        ...
        Atomics.store(
            this.write_ptr,
            0,
            (wr + to_write) % (this.@emu.joric.gwt.SharedQueue::_storage_capacity()())
        );
    ...
    public native int pop(Float32Array elements)/*-{
        ...
        var len = elements.length;
        var to_read = Math.min(this.@emu.joric.gwt.SharedQueue::_available_read(II)(rd, wr), len);

        var first_part = Math.min((this.@emu.joric.gwt.SharedQueue::_storage_capacity()()) - rd, to_read);
        ...
        Atomics.store(this.read_ptr, 0, (rd + to_read) % (this.@emu.joric.gwt.SharedQueue::_storage_capacity()()));
    ...
    private native int _available_write(int rd, int wr)/*-{
        return (this.@emu.joric.gwt.SharedQueue::capacity()()) - (this.@emu.joric.gwt.SharedQueue::_available_read(II)(rd, wr));
    }-*/;
