...
final class TeaVMBrowser {
    ...
    @JSFunctor
    interface DialogOptionCallback extends JSObject {
        void complete(boolean accepted, String value);
    }
    ...
    @JSBody(script = "var url = new URL(window.location.href); url.pathname = '/'; url.hash = ''; url.searchParams.delete('url'); url.searchParams.delete('rom'); return url.toString();")
    ...
    @JSBody(script = "if (!window.__joricDialog && typeof window.Dialog === 'function') { window.__joricDialog = new window.Dialog(); } return window.__joricDialog || null;")
    ...
    @JSBody(params = { "dialog", "message", "callback" }, script = "var html = String(message || '').replace(/(?:\\r\\n|\\r|\\n)/g, '<br>'); html = html.split('https://github.com/lanceewing/joric').join(\"<a href='https://github.com/lanceewing/joric' target='_blank'>https://github.com/lanceewing/joric</a>\"); if (!dialog || typeof dialog.alert !== 'function') { window.alert(message); callback(true, 'OK'); return; } dialog.alert('', { showStateButtons: false, template: '<b>' + html + '</b>' }).then(function(res) { if (res) { callback(true, res === true ? 'OK' : String(res)); } else { callback(false, null); } });")
    static native void showDialogAlert(JSObject dialog, String message, DialogAlertCallback callback);

    @JSBody(params = { "dialog", "title", "message", "options", "currentSelection", "callback" }, script = "if (!dialog || typeof dialog.promptForOption !== 'function') { callback(false, null); return; } dialog.promptForOption(title, message, options, currentSelection).then(function(res) { if (res && res.option) { callback(true, res.option); } else { callback(false, null); } });")
    static native void showDialogPromptForOption(JSObject dialog, String title, String message,
            String[] options, String currentSelection, DialogOptionCallback callback);
    ...
    @JSBody(params = { "callback" }, script = "var input = document.createElement('input'); input.type = 'file'; input.accept = '.tap,.dsk,.zip,.rom'; input.style.display = 'none'; document.body.appendChild(input);"
    ...
    @JSBody(params = { "callback" }, script = "var target = document.getElementById('embed-html') || document.body || document.documentElement;"
            + "var overlayId = 'joric-drop-overlay';"
            + "var overlay = document.getElementById(overlayId);"
            + "var dragDepth = 0;"
            + "if (!overlay) {"
            + "  overlay = document.createElement('div');"
            + "  overlay.id = overlayId;"
            + "  overlay.textContent = 'Drop an Oric file to load it';"
            ...
            + "}"
            + "if (target && !target.dataset.joricDropTargetStyled) {"
            ...
            + "  target.dataset.joricDropTargetStyled = 'true';"
            + "}"
